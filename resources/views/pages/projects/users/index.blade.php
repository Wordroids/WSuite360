<x-app-layout>
    ...
    <div class="flex-1 flex flex-col" x-data="getData">
        <div class="flex items-center justify-between">
            ...
            <button @click="addUserModal = true" class="bg-indigo-700 hover:bg-indigo-600 text-white p-3 rounded text-sm">Add User</button>
        </div>
                ...
                <tbody>
                    <template x-for="user in projectUsers">
                        <tr class="bg-white border-t">
                            <td class="px-6 py-4 text-sm text-gray-500 font-bold whitespace-nowrap">John Example Doe</td>
                            <td class="px-6 py-4 text-sm text-gray-800">john@example.com</td>
                            <td class="px-6 py-4 text-sm text-gray-800">Tester</td>
                            <td class="px-6 py-4 text-sm text-gray-800 flex space-x-2 justify-end">
                                <!-- Action Buttons -->
                                <button type="button" class="whitespace-nowrap text-xs border border-gray-600 px-3 py-1 rounded hover:bg-gray-500 hover:text-white transition-colors">
                                    Change Role
                                </button>
                                <button
        ...
        <!-- Add user modal -->
        <div class="relative z-10" x-show="addUserModal" x-cloak aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="fixed inset-0 bg-gray-500/75 backdrop-blur-sm transition-opacity" aria-hidden="true"></div>

            <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
                <div class="flex min-h-full justify-center p-4 text-center items-center">
                    <div class="relative transform rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all w-1/2" @click.outside="addUserModal = false">
                        <div class="flex items-center justify-between">
                            <div>
                            ...
                            <div>
                                ...
                                <button class="border border-gray-600 hover:bg-gray-600 hover:text-white p-3 rounded text-sm" @click="addUserModal = false">Close</button>
                            </div>
                        </div>
