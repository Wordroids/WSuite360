                ...
                <tbody>
                    <template x-for="user in taskUsers.data">
                        <tr class="bg-white border-t">
                            <td class="px-6 py-4 text-sm text-gray-500 font-bold whitespace-nowrap" x-text="user.name">
                                N/A</td>
            ...
            <div class="flex justify-end gap-2">
                <template x-for="(link, index) in taskUsers.links">
                    <button
                        ...
                        x-text="link.label.replace('&laquo;', '').replace('&raquo;', '')"
                        @click="fetchTaskUsers(link.url.split('page=')[1])" :disabled="!link.url"></button>
                </template>
            </div>
        </div>
                ...
                <div class="flex min-h-full justify-center p-4 text-center items-center">
                    <div class="relative transform rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all w-1/2"
                        ...
                        <table class="mt-6 min-w-full table-auto border-collapse border border-gray-200">
                            ...
                            <tbody>
                                <template x-for="user in availableUsers">
                                    <tr class="bg-white border-t">
    ...
    <div class="flex-1 flex flex-col" x-data="taskUserData">
        ...
        <div class="flex justify-end gap-2">
            <template x-for="(link, index) in taskUsers.links">
                <button
                    class="p-1 text-sm text-center border transition-colors border-gray-200 bg-white hover:bg-gray-600 hover:text-white rounded"
                    :class="{ 'bg-gray-600 text-white': link.active }" x-text="formatPaginationLabel(link.label)"
                    @click="fetchTaskUsers(link.url.split('page=')[1])" :disabled="!link.url"></button>
            </template>
        </div>
